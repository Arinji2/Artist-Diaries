...
function Signup() {
  ...
  const [errorNameParams, setErrorNameParams] = useState("");
  ...
  function nameErrorHelper(message: SetStateAction<string>) {
    setNameError(true);
    setErrorNameParams(message);
  }
  const errorHandling = () => {
    if (email === "") emailErrorHelper("Email cannot be empty");
    else if (password === "") passwordErrorHelper("Password cannot be empty");
    else if (name === "") nameErrorHelper("Name cannot be empty");
      ...
      .then((res) => {
        if (res.error) {
          ...
          if (res.error.message === "auth/invalid-email") {
            emailErrorHelper("Invalid Email");
          }
          if (res.error.message === "auth/wrong-password") {
            passwordErrorHelper("Wrong Password");
          }
        }
        if (res.data.user) {
  ...
  return (
    <div className="w-full h-fit md:h-[100svh] bg-[#3D4A4C] ">
      <div className="h-[30%] flex flex-col items-center justify-center">
        <h2 className="text-6xl text-white pt-32 font-righteous">Sign Up</h2>
      </div>
      <div className="w-full md:h-[70%] h-full flex md:flex-row items-center justify-evenly flex-col">
        <div className="h-full md:w-[50%] w-full flex flex-col items-start justify-around pl-10">
          ...
          <div className="w-full text-left md:mt-0 mt-5">
            <h2 className="text-3xl text-white font-righteous w-full">Name</h2>
            <input
              type={"text"}
              autoCapitalize="off"
              autoComplete="off"
            ...
            >
              {nameError ? "Error: ".concat(errorNameParams) : "No Error"}
            </p>
          </div>
          <div
